...
**Model Catalog**:
A Provider's retained set of Upstream Models a caller may ask for, assembled from Provider discovery, Provider Template knowledge, and Owner additions, then narrowed by Owner exclusions. It retains allow-listed metadata against each exact Upstream Model ID without changing routing identity, including token limits, modalities, and explicit chat support evidence; Provider-reported facts may be supplemented by unambiguous external catalog facts when fields are missing. Owner capability overrides remain authoritative. It is what the Provider Directory reports. It says nothing about which Upstream Key can serve a given model.
